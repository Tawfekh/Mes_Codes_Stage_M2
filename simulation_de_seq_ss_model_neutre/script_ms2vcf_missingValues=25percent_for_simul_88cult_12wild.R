...
while (i <= nrep) {
  ...
  second_line_wild <- t(c("#CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT",(paste("Ind",rep(1:12),sep="_"))))
  ...
  second_line_cult <- t(c("#CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT",(paste("Ind",rep(1:(nsam-12)),sep="_"))))
...
for(k in 1:segsite){
      locus_linewild <- t(c("chr1",position[k],".",ref,alt,".",".",".","GT", mat[k,c(1:12)]))
	...
	write.table(locus_linewild, file=paste("./25wild_vcf/outvcf_rep",i,".vcf",sep=""), sep="\t", quote=FALSE, col.names=F, row.names=F, append=TRUE)
      ...
      locus_line <- t(c("chr1",position[k],".",ref,alt,".",".",".","GT", mat[k,c(13:nsam)]))
